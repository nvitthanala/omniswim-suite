...
function ManagerSkeleton({ label }: { label?: string }) {
  return (
...
function SuiteSkeleton({ label }: { label?: string }) {
  return (
...
const SKELETON_BY_KIND: Record<AppletSkeletonKind, (props: { label?: string }) => ReactElement> = {
  manager: ManagerSkeleton,
...
export function AppletSkeleton({ kind = 'suite', label }: AppletSkeletonProps) {
  const Skeleton = SKELETON_BY_KIND[kind];
