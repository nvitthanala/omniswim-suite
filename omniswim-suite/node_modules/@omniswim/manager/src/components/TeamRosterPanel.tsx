...
function AthleteRoleTag({ role, isRecruit }: { role: ScorerRosterAthleteRole; isRecruit?: boolean }) {
  if (isRecruit) {
    return (
      <span className="shrink-0 px-1 py-0.5 rounded text-[8px] font-medium uppercase tracking-wide text-[var(--text-accent)] border border-[var(--text-accent)]/30">
        Recruit
      </span>
    );
  }
...
export default function TeamRosterPanel({
  ...
  const rosterTable = (
    <div className={expanded ? 'flex flex-col flex-1 min-h-0' : undefined}>
          ...
          {editable && workspace && onWorkspaceUpdate ? (
            <>
              <button
                ...
                className="text-[9px] text-[var(--text-accent)] hover:underline flex items-center gap-1 uppercase"
              ...
              <button
                ...
                className="text-[9px] text-theme-secondary hover:text-[var(--text-accent)] flex items-center gap-1 uppercase"
              >
                All teams
              </button>
            </>
      ...
      <p className="text-[10px] text-theme-secondary my-3 leading-relaxed">
        ...
        <table className="w-full">
          ...
          <tbody>
            {!selectedTeam || teamRows.length === 0 ? (
              <tr>
                <td colSpan={colSpan + 1} className="py-4 text-center text-[10px] text-theme-muted italic">
                    ...
                    <td className="py-1.5 px-2" title={row.name}>
                      <div className="flex items-center gap-1.5 min-w-0">
                        ...
                        {entryOver.individualOver || entryOver.relayOver ? (
                          <span className="text-[7px] text-amber-400 shrink-0" title="Over entry limit">
                            !
                          </span>
                        ) : null}
                      </div>
                      {profile && profile.primaryEvents.length > 0 ? (
                        <p className="text-[8px] text-theme-muted truncate mt-0.5" title={profile.primaryEvents.join(', ')}>
                          {profile.primaryEvents.slice(0, 3).join(' · ')}
                        </p>
                      ) : null}
                    </td>
                    <td className="py-1.5 px-2 text-right text-theme-secondary text-[10px]">
                      {row.classYear || '—'}
                    </td>
                    <td
