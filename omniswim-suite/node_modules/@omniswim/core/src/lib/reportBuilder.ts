...
import { buildSeasonTrends } from './seasonAnalytics';

function esc(s: string): string {
  return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

export function buildMeetReportHtml(workspace: Workspace): string {
  ...
  const swimmerRows = trends.swimmerTrends
    .slice(0, 50)
    .map(
...
<body>
  ...
  <p class="meta">Omni Swim Suite · Generated ${esc(generated)} · ${menCount} men&apos;s + ${womenCount} women&apos;s results</p>
